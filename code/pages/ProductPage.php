    ...
    public function PurchaseForm()
    {
        ...
        if ($data->Available) {
            ...
            foreach ($groupedBy as $id => $set) {
                ...
                $fullOptions = array();
                foreach ($set as $item) {
                    $fullOptions[ProductPage::getGeneratedValue($data->Code, $group->Title, $item->getGeneratedValue(),
                        'value')] = $item->getGeneratedTitle();
                    if (!$item->Availability) {
                        array_push($disabled,
                            ProductPage::getGeneratedValue($data->Code, $group->Title, $item->getGeneratedValue(),
                                'value'));
                    }
                }
                $optionsSet->push(
            ...
            $submit->setAttribute('name',
                ProductPage::getGeneratedValue($code, 'Submit', _t('ProductForm.AddToCart', 'Add to Cart')));
        ...
        $form->setAttribute('action', FoxyCart::getFormActionURL());
