...
class FoxyCart_Helper
{
    ...
    public static function fc_hash_querystring($qs, $output = true)
    {
        ...
        preg_match_all('%(?P<amp>&(?:amp;)?)(?P<prefix>[a-z0-9]{1,3}:)?(?P<name>[^=]+)=(?P<value>[^&]+)%', $qs, $pairs,
            PREG_SET_ORDER);
        ...
        foreach ($pairs as $pair) {
            ...
            if (in_array($pair['name'], self::$cart_excludes) || in_array($pair['prefix'],
            ...
            $value = self::fc_hash_value($codes[$pair['prefix']], $pair['name'], $pair['value'], 'value', false,
                'urlencode');
    ...
    public static function fc_hash_html($html)
    {
        ...
        preg_match_all('%<a .*?href=[\'"]' . preg_quote(self::$cart_url) . '(?:\.php)?\?(.+?)[\'"].*?>%i', $html,
            $querystrings);
        ...
        foreach ($querystrings[1] as $querystring) {
            ...
            $pattern = '%(href=[\'"])' . preg_quote(self::$cart_url, '%') . '(?:\.php)?\?' . preg_quote($querystring,
                    '%') . '([\'"])%i';
        ...
        preg_match_all('%<form [^>]*?action=[\'"]' . preg_quote(self::$cart_url) . '?[\'"].*?>(.+?)</form>%is', $html,
            $forms);
        foreach ($forms[1] as $form) {
            $count['forms']++;
            self::$log[] = '<strong>Signing form</strong> with data: ' . htmlspecialchars(substr($form, 0,
                    150)) . '...';
            ...
            foreach ($codes as $code) {
                ...
                preg_match_all('%<input [^>]*?name=([\'"])' . preg_quote($prefix) . '(?![0-9]{1,3})(?:.+?)\1[^>]*>%i',
                    $form, $inputs);
                foreach ($inputs[0] as $input) {
                    $count['inputs']++;
                    // Test to make sure both name and value attributes are found
                    if (preg_match('%name=([\'"])' . preg_quote($prefix) . '(?![0-9]{1,3})(.+?)\1%i', $input,
                            $name) > 0
                    ) {
                        ...
                        // Skip the cart excludes
                        if (in_array($prefix . $name[2], self::$cart_excludes) || in_array(substr($prefix . $name[2], 0,
                                2), self::$cart_excludes_prefixes)
                        ) {
                        ...
                        if ($type[2] == 'radio') {
                            $input_signed = preg_replace('%([\'"])' . preg_quote($value[2]) . '\1%',
                                '${1}' . self::fc_hash_value($code, $name[2], $value[2], 'value', false) . "$1",
                                $input);
                        } else {
                            $input_signed = preg_replace('%([\'"])' . $prefix . preg_quote($name[2]) . '\1%',
                                '${1}' . $prefix . self::fc_hash_value($code, $name[2], $value[2], 'name',
                                    false) . "$1", $input);
                        }
                ...
                preg_match_all('%<select [^>]*name=([\'"])' . preg_quote($prefix) . '(?![0-9]{1,3})(.+?)\1[^>]*>(.+?)</select>%is',
                    $form, $lists, PREG_SET_ORDER);
                foreach ($lists as $list) {
                    $count['lists']++;
                    preg_match_all('%<option [^>]*value=([\'"])(.+?)\1[^>]*>(?:.*?)</option>%i', $list[0], $options,
                        PREG_SET_ORDER);
                    self::$log[] = '<strong>Options:</strong> <pre>' . htmlspecialchars(print_r($options,
                            true)) . '</pre>';
                ...
                preg_match_all('%<textarea [^>]*name=([\'"])' . preg_quote($prefix) . '(?![0-9]{1,3})(.+?)\1[^>]*>(.*?)</textarea>%is',
                    $form, $textareas, PREG_SET_ORDER);
                // echo "\n\nTextareas: ".print_r($textareas, true);
                foreach ($textareas as $textarea) {
                    $count['textareas']++;
                    // Tackle implied "--OPEN--" first, if textarea is empty
                    $textarea[3] = ($textarea[3] == '') ? '--OPEN--' : $textarea[3];
                    $textarea_signed = preg_replace('%([\'"])' . preg_quote($prefix . $textarea[2]) . '\1%',
                        "$1" . self::fc_hash_value($code, $textarea[2], $textarea[3], 'name', false) . "$1",
                        $textarea[0]);
                ...
                $form = preg_replace('%<button ([^>]*)name=([\'"])(.*?)\1([^>]*>.*?</button>)%i',
                    "<button $1name=$2x:$3$4", $form);
